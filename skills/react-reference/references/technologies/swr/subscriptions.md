...
Domain operation скрывает adapter, mapper, source errors и технический transport. Внутри неё удаляй тот же handler,
который был передан transport. Не используй broad `off(event)` и не отключай shared connection при unmount одного hook.
...
Domain-owned outbound command выполняй через public facet доменного юнита. Socket client/service напрямую
