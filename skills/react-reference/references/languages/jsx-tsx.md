...
Применяй к JSX/TSX-деревьям. Для `.jsx` также загрузи [JavaScript](javascript.md), для `.tsx` — [JavaScript](javascript.md) и [TypeScript](typescript/README.md). Владельца и границу компонента определяй по [`architecture/README.md`](../architecture/README.md) и [`architecture/ui-units.md`](../architecture/ui-units.md).
...
`OrdersSection` размещается в отдельном component-файле у ближайшего unit owner. Не создавай новый юнит только ради декомпозиции; критерии самостоятельной UI-ответственности находятся в [`architecture/ui-units.md`](../architecture/ui-units.md).
