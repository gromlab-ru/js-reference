...
Этот документ помогает выбрать источник истины, владельца lifecycle и React-механизм доступа к состоянию. Архитектурного владельца и публичный фасет определяй по [`architecture/README.md`](../architecture/README.md).
...
| Данные получены REST GET и отражают состояние API | SWR | Cache, request state, deduplication и revalidation в одном lifecycle server state |
...
Точные правила server state применяй по [`technologies/swr/get-data.md`](../technologies/swr/get-data.md) и [`technologies/swr/subscriptions.md`](../technologies/swr/subscriptions.md).
...
SWR хранит клиентскую проекцию server state. Zustand не должен зеркалировать её.
