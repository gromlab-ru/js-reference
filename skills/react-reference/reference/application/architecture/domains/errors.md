...
Успешная операция возвращает доменный результат напрямую. Ожидаемый неуспешный исход бросается как typed domain exception. Публичный adapter не передаёт consumer исходную ошибку API client, SDK или transport.
...
Значение, которое не прошло guard, обрабатывается по [`failure-handling`](../../quality/failure-handling.md). Не добавляй code `UNEXPECTED` в domain error union.

## Граница adapter

Каждый adapter определяет полный набор известных source outcomes для своей операции:

1. Успешный source response преобразуется в domain result.
2. Ожидаемый source outcome преобразуется в зафиксированную domain error.
3. Неизвестная ошибка преобразуется в `ApplicationDefect`.

GET hook сохраняет тот же контракт ошибок, что его GET adapter. Hook может разделить распознанную domain error и неизвестный defect для React consumer, но не создаёт новый набор error codes и не публикует source error.
