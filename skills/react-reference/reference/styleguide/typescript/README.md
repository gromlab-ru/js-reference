...
```ts
/**
 * Properties карточки пользователя.
 */
...
Каждый `type`, `interface`, `enum` и каждое их поле или значение имеют многострочный JSDoc. Это правило действует также
для внутренних псевдонимов и полей. Константу документируй, если это публичный контракт, предметное ограничение,
неочевидное значение или переиспользуемая конфигурация.

Правило относится к коду, который команда пишет вручную. Не редактируй созданные генератором файлы ради формата JSDoc:
их оформление определяется генератором, а каталог целиком заменяется при повторном создании.
...
- Типы, интерфейсы, перечисления и их поля или значения используют многострочный JSDoc.
