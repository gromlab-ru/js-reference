...
Для REST в React SPA используй технологию REST API и библиотеку SWR:

- [`REST API`](../../technologies/rest-api.md) определяет направление взаимодействия с REST-сервисами;
- [`SWR GET`](../../libraries/swr/get-data.md) связывает GET-operation с React lifecycle, cache identity, request state и revalidation.
...
Hook использует SWR и вызывает предметную operation владельца, но внешний consumer не знает API client, URL, DTO и SWR key implementation. Семантику keys, cache и revalidation определяет [`libraries/swr/get-data.md`](../../libraries/swr/get-data.md).
...
Фактические имена и аргументы определяет предметный контракт. Внутренний adapter вызывает API client и преобразует source response и errors. После успешной mutation синхронизируй связанные SWR GET keys через update, optimistic update или revalidation по [`libraries/swr/get-data.md`](../../libraries/swr/get-data.md#revalidation-после-mutation).
