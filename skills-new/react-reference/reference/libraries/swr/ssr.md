...
Server Component импортирует server-safe key generator и operation из server-compatible public facet доменного юнита.
Server API client используй напрямую только для данных без доменного владельца:
...
Имена operations и компонентов условны. Operation должна быть server-safe и использовать подходящий технический
transport из `infra` внутри доменного юнита. Если интеграция зависит от request context, импортируй capability из server
facet доменного юнита. Не импортируй browser operation или
