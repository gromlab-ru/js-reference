...
Используй `useSWR` для REST GET-data, которая представляет server state и участвует в React render. Для предметных
данных fetcher вызывает operation из public facet доменного юнита; для данных без доменного владельца — готовую GET-operation API client по
технологии [`REST API`](../../technologies/rest-api.md).

```text
useSWR
→ operation из public facet доменного юнита или GET-operation API client
→ HttpClient
→ REST API
```
...
- Изменяющие operations выполняй вне SWR. Operation над предметными данными вызывается через public facet доменного
  юнита, а operation для данных без доменного владельца — напрямую через API client из event handler, action или владельца сценария.
- После изменения синхронизируй связанные GET keys через `mutate`.
- Read-operation через `POST` по умолчанию также выполняй вне SWR. Используй operation из public facet доменного юнита
  либо API client для данных без доменного владельца. Operation не становится GET-cache только
  из-за того, что не изменяет данные.

Императивный GET для download, export или другого результата, который не является server state для render, выполняй
через public facet доменного юнита при наличии доменного владельца, иначе — через API
...
identity. Fetcher передаёт operation только параметры ресурса, а JWT или cookie
...
Fetcher остаётся локальной функцией hook и выполняет operation через public facet доменного юнита:
...
Не создавай в fetcher самостоятельный `fetch`, URL, auth headers или общую обработку HTTP-ошибок. Для предметных данных
их скрывает доменная operation, а для данных без доменного владельца эти правила принадлежат `HttpClient`.
...
public facet доменного юнита для отдельного consumer. Передай в fetcher готовую domain operation:

```ts
const fetcher = ([, petId]: GetPetKey) => getPet(petId)
```

Public operation реализуется внутренним adapter доменного юнита. Mappers, adapters и интерпретация source errors
остаются внутри домена, а технический HTTP transport принадлежит `infra`. SWR отвечает только за key, cache и React
lifecycle.
...
Domain hook принадлежит доменному юниту вместе с его контрактом, errors, mappers и internal adapters. Внешние consumers
получают hook через подходящий public facet юнита; не размещай его во внешнем общем каталоге hooks.
...
- Fetcher выполняет operation из public facet доменного юнита или GET-operation API client для данных без доменного владельца.
- Key содержит namespace и все влияющие на результат аргументы.
- Remote data не копируются в другой client store.
- Domain mutation выполняется через public facet доменного юнита; mutation для данных без доменного владельца выполняется API client.
