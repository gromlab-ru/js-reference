...
Этот документ определяет подключение realtime к React lifecycle. Технический контракт keys, subscribe callback, cleanup, sync payload, auth scope и reconnect применяй по [`technologies/swr/subscriptions.md`](../../../technologies/swr/subscriptions.md).
...
| Нужно отправить предметную command | Action вызывает operation публичного фасета домена |
...
Подход подходит для presence, live progress и аналогичных значений. Не копируй latest value в React state или Zustand только ради нескольких consumers: одинаковый subscription key уже предоставляет shared projection.

Если обязательна обработка каждого события, latest-value semantics не подходит. Используй queue, reducer или event store по критериям [`technologies/swr/subscriptions.md#когда-swr-не-подходит`](../../../technologies/swr/subscriptions.md#когда-swr-не-подходит).
...
GET предоставляет данные до первого события и восстанавливает проекцию после disconnect. Lifecycle owner применяет snapshot, delta или invalidation по domain contract и защищает cache от stale updates. Стратегии определены в [`technologies/swr/subscriptions.md#стратегии-sync`](../../../technologies/swr/subscriptions.md#стратегии-sync).
...
- Cleanup, reconnect и stale update policy соответствуют `technologies/swr/subscriptions.md`.
